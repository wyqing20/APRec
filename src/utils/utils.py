...
def format_metric(result_dict: Dict[str, Any]) -> str:
    ...
    if 'AUC' in result_dict:
        format_str.append('{}:{:<.4}'.format('AUC', result_dict['AUC']))
    if 'DIS_AUC' in result_dict:
        format_str.append('{}:{:<.4}'.format('DIS_AUC', result_dict['DIS_AUC']))
    if 'acc' in result_dict:
        format_str.append('{}:{:<.4}'.format('ACC', result_dict['acc']))
    if 'f1_macro' in result_dict:
        format_str.append('{}:{:<.4}'.format('F1_Macro', result_dict['f1_macro']))
        
    metrics = np.unique([k.split('@')[0] for k in result_dict.keys() if 'NDCG'  in k or 'HR' in k or "MRR" in k])
    topks = np.unique([int(k.split('@')[1]) for k in result_dict.keys()  if 'NDCG'  in k or 'HR' in k or "MRR" in k])
    for topk in np.sort(topks):
        for metric in np.sort(metrics):
            ...
            if type(m) is float or type(m) is np.float or type(m) is np.float32 or type(m) is np.float64:
                format_str.append('{}:{:<.4}'.format(name, m))
