...
class AttackAliECReader2(BaseReader):
    ...
    def _read_data(self) -> NoReturn:
        ...
        # p_keys=[ 'warm_cold_zero_shot_train4','warm_cold_zero_shot_train2','zero_shot_dev2','zero_shot_test2']
        # for key in p_keys:
        #     self.data_df[key] = pd.read_csv(os.path.join(self.prefix, self.dataset, key + '.csv'), sep=self.sep)
        #     self.data_df[key] = utils.eval_list_columns(self.data_df[key])
        ...
        # p_keys=['warm_cold_zero_shot_train','warm_one_shot_train','warm_one_shot_dev']
        # for key in p_keys:
        #     self.data_df[key] = pd.read_csv(os.path.join(self.prefix, self.dataset, key + '.csv'), sep=self.sep)
        #     self.data_df[key] = utils.eval_list_columns(self.data_df[key])
        # logging.info('Counting zero shot dataset statistics...')
        # self.all_df = pd.concat([self.data_df[k][['user_id', 'item_id', 'time','old','gender','power']] for k in p_keys])
        # self.n_users, self.n_items = len(self.all_df['user_id'].unique()), len(self.all_df['item_id'].unique())
        # logging.info('"warm one shot # user": {}, "# item": {}, "# entry": {}'.format(
        #     self.n_users , self.n_items , len(self.all_df)))

        # p_keys=[ 'warm_zero_shot_train','warm_zero_shot_dev']
        # for key in p_keys:
        #     self.data_df[key] = pd.read_csv(os.path.join(self.prefix, self.dataset, key + '.csv'), sep=self.sep)
        #     self.data_df[key] = utils.eval_list_columns(self.data_df[key])
        # logging.info('Counting zero shot dataset statistics...')
        # self.all_df = pd.concat([self.data_df[k][['user_id', 'item_id', 'time','old','gender','power']] for k in p_keys])
        # self.n_users, self.n_items = len(self.all_df['user_id'].unique()), len(self.all_df['item_id'].unique())
        # logging.info('"warm zero shot # user": {}, "# item": {}, "# entry": {}'.format(
        #     self.n_users , self.n_items , len(self.all_df)))
    ...
    def _append_his_info(self) -> NoReturn:
        ...
        for key in ['train', 'dev','cold_train','cold_dev','cold_test']:
            df = self.data_df[key]
            position = list()
