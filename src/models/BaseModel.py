...
class BaseModel(nn.Module):
    ...
    #     elif 'Embedding' in str(type(m)):
    #         nn.init.xavier_uniform_(m.weight)
# dev=(AUC:0.7095,ACC:0.3529  AUC:0.9019,HR@10:0.7324,NDCG@10:0.5212) [7.6 s] *
    ...
    def load_model(self, model_path=None) -> NoReturn:
        if model_path is None:
            model_path = self.model_path
        # model_path='../old_model/CrossDomainRS/CrossDomainRS_old/CrossDomainRS__AliCDR__0__lr=0.0001__l2=1e-06__emb_size=64__num_layers=1__num_heads=1__f_encoder=Linear__stage=21__history_max=100.pt'
    ...
    class Dataset(BaseDataset):
        def __init__(self, model, corpus, phase: str):
            ...
            print(phase,corpus.data_df.keys(),self.phase)
...
class GeneralModel(BaseModel):
    ...
    class Dataset(BaseModel.Dataset):
        def _get_feed_dict(self, index):
            ...
            # if user_id>80964 and target_item<=87896:
            #     print('error')
            # elif user_id<=80964 and target_item>87896:
            #     print('error')
                
            if 'train' not in self.phase and (self.model.test_all) :
                
                neg_items = np.arange(1, 2)
            else:
...
class SequentialModel(GeneralModel):
    ...
    class Dataset(GeneralModel.Dataset):
        ...
        def _get_feed_dict(self, index):
            ...
            # if pos ==0 and self.model.stage==3:## just for padding to slove zero-shot compution
            #     feed_dict['history_items'] = np.array([0]) 
            #     feed_dict['history_times'] = np.array([0])
            #     feed_dict['lengths'] = len(feed_dict['history_items'])
            #     feed_dict['position']=pos
            return feed_dict
